...
const firebaseConfig = {
  ...
  databaseURL: "https://live-3d-607ed-default-rtdb.europe-west1.firebasedatabase.app"
};
...
db.ref('sequenceActive').on('value', (snapshot) => {
  ...
  if(isPlaying){
    db.ref('playingIndex').get().then((snapshot) => {
...
function play_audio(){
    ...
    if (audios_files[playing_index] && audios_files[playing_index].startsWith(screen)) {
